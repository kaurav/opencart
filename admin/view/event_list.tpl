            ...
            <table class="table table-bordered table-hover">
              ...
              <tbody>
                ...
                <tr>
                 ...
                 <!--  <td class="text-left"><?php echo $event['model']; ?></td>
                  <td class="text-right"><?php if ($event['special']) { ?>
                    <span style="text-decoration: line-through;"><?php echo $event['price']; ?></span><br/>
                    <div class="text-danger"><?php echo $event['special']; ?></div>
                    <?php } else { ?>
                    <?php echo $event['price']; ?>
                    <?php } ?></td>
                  <td class="text-right"><?php if ($event['quantity'] <= 0) { ?>
                    <span class="label label-warning"><?php echo $event['quantity']; ?></span>
                    <?php } elseif ($event['quantity'] <= 5) { ?>
                    <span class="label label-danger"><?php echo $event['quantity']; ?></span>
                    <?php } else { ?>
                    <span class="label label-success"><?php echo $event['quantity']; ?></span>
                    <?php } ?></td> -->
...
$('#button-filter').on('click', function() {
	var url = 'index.php?route=catalog/event&token=<?php echo $token; ?>';
...
$('input[name=\'filter_model\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=catalog/event/autocomplete&token=<?php echo $token; ?>&filter_model=' +  encodeURIComponent(request),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
