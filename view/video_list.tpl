          ...
          <div class="row">
            <div class="col-sm-4">
              <div class="form-group">
                ...
                <input type="text" name="filter_title" value="<?php echo $filter_title; ?>" placeholder="<?php echo $entry_name; ?>" id="input-name" class="form-control" />
             <!--  </div>
            <div class="col-sm-4"> -->
             <!--  <div class="form-group">
              ...
              <tbody>
                ...
                <tr>
                  <td class="text-center"><?php if (in_array($video['video_id'], $selected)) { ?>
                    ...
                    <?php } ?></td>
                  <td class="text-center"><?php if ($video['image']) { ?>
                    <img src="<?php echo $video['image']; ?>" alt="<?php echo $video['name']; ?>" class="img-thumbnail" />
                    <?php } else { ?>
                    <span class="img-thumbnail list"><i class="fa fa-camera fa-2x"></i></span>
                    <?php } ?></td>
                  <td class="text-left"><?php echo $video['title']; ?></td>
<?php /*
                  <td class="text-left"><?php echo $video['status']; ?></td> */?>
...
$('#button-filter').on('click', function() {
  var url = 'index.php?route=catalog/video&token=<?php echo $token; ?>';
...
$('input[name=\'filter_title\']').autocomplete({
  'source': function(request, response) {
    $.ajax({
      url: 'index.php?route=catalog/video/autocomplete&token=<?php echo $token; ?>&filter_title=' +  encodeURIComponent(request),
      dataType: 'json',
  ...
  'select': function(item) {
    $('input[name=\'filter_title\']').val(itemm['label']);
  }
});
